def sourceInfo = null
try {
    println("Attempting to load shared lib from branch: ${env.BRANCH_NAME} -- Using bitbucket-creds-${TENANT}")

    if ("${TOOLS_URL}".contains('git'))
        sourceInfo = [$class: 'GitSCMSource', remote: "${TOOLS_URL}"]
    else
        sourceInfo = [$class: 'GitSCMSource', remote: "${TOOLS_URL}", credentialsId: "bitbucket-creds-${TENANT}"]
